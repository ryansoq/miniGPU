...
void drawMeshShaderISA(const Mesh &mesh, const Mat4 &model, const Mat4 &view,
  ...
  for (size_t i = 0; i < mesh.pos.size(); i++) {
    Vec4 clip = mvp * Vec4{mesh.pos[i].x, mesh.pos[i].y, mesh.pos[i].z, 1};
    float w = clip.w == 0 ? 1e-6f : clip.w;
    vs[i].ndc = {clip.x / w, clip.y / w, clip.z / w};
    Vec4 n = model * Vec4{mesh.normal[i].x, mesh.normal[i].y, mesh.normal[i].z, 0};
    vs[i].nrm = normalize(Vec3{n.x, n.y, n.z});
  }
  for (size_t t = 0; t + 2 < mesh.idx.size(); t += 3) {
    ...
    for (int y = miny; y <= maxy; y++)
      for (int x = minx; x <= maxx; x++) {
        ...
        fb.depth[di] = z;
        // ★ 這一步：把內插法向量餵進 shader 的 input，跑真 ISA
        Vec3 nrm = normalize(A.nrm * b0 + B.nrm * b1 + C.nrm * b2);
        GpuState st;
        st.I[0] = nrm.x; st.I[1] = nrm.y; st.I[2] = nrm.z;
        if (!shader_run(program, st)) continue;
        uint8_t *p = &fb.rgba[di * 4];
        p[0] = to8(st.O[0]); p[1] = to8(st.O[1]);
        p[2] = to8(st.O[2]); p[3] = 255;
      }
  }
}
