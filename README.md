...
| **gl**（圖形 API/驅動） | `gl/` | 上傳 proj/view/model 三顆矩陣、綁 shader，開動管線 |
...
**矩陣走真引擎版**：CPU 只各自準備 `model` / `view` / `proj` 三顆（不預先合成
MVP），gl 分開上傳，vertex shader 自己做 `proj*(view*(model*pos))`。這跟姊妹專案
[minivk](https://github.com/ryansoq/minivk) 共用同一顆 ToyGPU（同暫存器佈局、同 shader
binary）。想看「CPU 先合成一顆 `mvp`、shader 只做 `mvp*pos`」的最簡捷徑版，見
[ToyGPU](https://github.com/ryansoq/ToyGPU) v1。
