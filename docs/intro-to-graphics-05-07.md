...
### Projection 投影矩陣：透視 vs 正交

- **Perspective 透視**：模擬人眼，近大遠小。由 `fov, aspect, near, far` 定出視錐（frustum）。
  它在 `w` 分量塞入深度資訊，讓步驟③ 的 ÷w 產生縮放。
- **Orthographic 正交**：平行投影，遠近一樣大（工程製圖、2D 遊戲、UI）。

投影矩陣做兩件事：把視錐壓成 -1~1 的標準方塊 + 為透視除法鋪路。
